 ...
 * The VAPI widget embedded in index.html handles all voice call functionality
 */

// Simple click handler for the voice button - let VAPI widget handle the actual calls
if (voiceWaveBtn) {
  voiceWaveBtn.addEventListener('click', () => {
    console.log('Voice button clicked - VAPI widget will handle the call');
    // The VAPI widget will automatically handle the call initiation
    // No custom code needed here since we're using the embed widget
  });
}
...
window.addEventListener('load', () => {
  ...
  setTimeout(() => {
    if (heroSubtitle) heroSubtitle.style.animation = 'slideInUp 0.8s ease forwards';
  }, 400);
